...
import { CreditPayoutsManager } from "@/components/admin/CreditPayoutsManager";
...
function StoreSalesPage() {
  const [mainTab, setMainTab] = useState<"all_sales" | "credit_payouts">("all_sales");
  ...
  return (
    <AdminLayout title="Vendas das Lojas" subtitle="Acompanhe pedidos do marketplace e faça a gestão de repasses aos lojistas">
      <Tabs value={mainTab} onValueChange={(v: any) => setMainTab(v)} className="space-y-6">
        <TabsList className="bg-muted/60 border border-border/50 rounded-2xl p-1.5 gap-1.5 h-auto flex-wrap">
          <TabsTrigger value="all_sales" className="rounded-xl text-xs font-black data-[state=active]:shadow-sm">
            <ShoppingBag className="h-3.5 w-3.5 mr-1.5" />
            Todas as Vendas
          </TabsTrigger>
          <TabsTrigger value="credit_payouts" className="rounded-xl text-xs font-black data-[state=active]:bg-amber-500 data-[state=active]:text-white">
            <Coins className="h-3.5 w-3.5 mr-1.5" />
            Repasses de Vendas em Créditos
          </TabsTrigger>
        </TabsList>

        <TabsContent value="credit_payouts" className="space-y-6">
          <CreditPayoutsManager />
        </TabsContent>

        <TabsContent value="all_sales" className="space-y-6">
          {/* Metrics Row */}
